...
const App = {
  ...
  async fetchInitialData() {
    try {
      ...
      if (res.ok) {
        ...
        this.state.budgetItems = data.budgetItems || [];
  ...
  populateDropdowns() {
    ...
    // Populate budget items dropdowns
    const budgetItemSelects = ['nr-budget-item', 'er-budget-item'];
    budgetItemSelects.forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      const prevVal = el.value;
      el.innerHTML = '<option value="">-- Bütçe Kalemi Seçiniz (Opsiyonel) --</option>';
      const items = (this.state.budgetItems || []).filter(b => b.isActive !== false);
      items.forEach(b => {
        el.innerHTML += `<option value="${b.name}" data-code="${b.code}">${b.code} - ${b.name}</option>`;
      });
      if (prevVal && Array.from(el.options).some(o => o.value === prevVal)) {
        el.value = prevVal;
      }
  ...
  // 1. DASHBOARD RENDERER (SADE, ŞIK VE MODERN YÖNETİCİ KOKPİTİ)
  renderDashboard() {
    ...
    const allNotifs = this.getAllNotifications ? this.getAllNotifications() : [];
    const activeDemands = requests.filter(r => r.status !== 'Tamamlandı' && r.status !== 'Reddedildi' && r.status !== 'İptal');

    // 1.1 Üst Karşılama ve Durum Kartı
    const curUser = this.state.currentUser;
    const userName = curUser?.name || 'Değerli Kullanıcı';
    const welcomeEl = document.getElementById('dash-welcome-user');
    const welcomeSubEl = document.getElementById('dash-welcome-subtext');
    const quickNotifCountEl = document.getElementById('dash-quick-notif-count');

    if (welcomeEl) welcomeEl.innerText = `Hoş Geldiniz, ${userName}`;
    if (quickNotifCountEl) quickNotifCountEl.innerText = allNotifs.length;
    if (welcomeSubEl) {
      if (activeDemands.length === 0 && allNotifs.length === 0) {
        welcomeSubEl.innerText = 'Harika! Şu an bekleyen acil bir işlem veya bildirim bulunmuyor.';
      } else {
        welcomeSubEl.innerText = `Bugün sistemde aktif ${activeDemands.length} talep işlem görüyor, ${allNotifs.length} bildirim ve aksiyon bekleyen durum mevcut.`;
      }
    }

    // 1.2 4 Temel Stratejik Finans & Hız Göstergesi
    this.renderDashboardMacroKPIs(requests, contracts, invoices, guarantees);

    // 1.3 İşlemdeki Aktif Talepler Listesi (Sol Kolon)
    this.renderDashboardActiveRequestsList(requests);

    // 1.4 Hızlı Durum & Kritik Uyarılar (Sağ Kolon)
    this.renderDashboardActionAlerts(requests, contracts, invoices, guarantees, allNotifs);
  },
    ...
    requests.forEach(r => {
      ...
      if (startDateStr && endDateStr) {
        ...
        if (!isNaN(dStart.getTime()) && !isNaN(dEnd.getTime()) && dEnd >= dStart) {
          totalTurnaroundDays += Math.ceil((dEnd - dStart) / (1000 * 60 * 60 * 24));
  ...
  renderDashboardActiveRequestsList(requests) {
    const listEl = document.getElementById('dash-active-requests-list');
    if (!listEl) return;

    // Filter active/in-progress requests (or recent if none active)
    let displayReqs = requests.filter(r => r.status !== 'Tamamlandı' && r.status !== 'Reddedildi' && r.status !== 'İptal');
    if (displayReqs.length === 0) {
      displayReqs = [...requests].slice(0, 5);
    } else {
      // Sort by SLA delay or most recent
      displayReqs = displayReqs.sort((a, b) => (b._diffDays || 0) - (a._diffDays || 0)).slice(0, 5);
    }

    if (displayReqs.length === 0) {
      listEl.innerHTML = `
        <div style="padding: 2rem; text-align: center; color: var(--text-muted); font-size: 0.85rem; border: 1px dashed var(--border-color); border-radius: var(--radius-md);">
          ✨ Şu anda sistemde bekleyen aktif talep kaydı bulunmamaktadır.
        </div>
      `;
      return;
    }

    listEl.innerHTML = displayReqs.map(r => {
      const amt = r.actualAmount > 0 ? r.actualAmount : (r.budgetAmount || r.estimatedAmount || 0);
      const isOverdue = r._diffDays >= 14 && r.status !== 'Tamamlandı';

      return `
        <div class="dash-compact-request-row" onclick="App.openEditModal('${r.id}')" title="Detayları İncele (#${r.requestBarcode || r.id})">
          <div style="display: flex; align-items: center; gap: 0.65rem; min-width: 0; flex: 1;">
            <span style="font-family: var(--font-mono); font-weight: 800; color: var(--accent-primary); font-size: 0.82rem; background: rgba(59, 130, 246, 0.08); padding: 0.2rem 0.45rem; border-radius: 4px; flex-shrink: 0;">
              #${r.requestBarcode || r.id}
            </span>
            <div style="min-width: 0; flex: 1;">
              <div style="font-size: 0.85rem; font-weight: 700; color: var(--text-main); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                ${r.subject || 'Konu Belirtilmemiş'}
              </div>
              <div style="font-size: 0.73rem; color: var(--text-muted); display: flex; gap: 0.5rem; align-items: center; margin-top: 0.15rem;">
                <span style="font-weight: 600; color: var(--text-main);">🏢 ${r.unit || '-'}</span>
                <span>•</span>
                <span>👤 ${r.assignedTo || 'Atanmadı'}</span>
                ${isOverdue ? `<span style="color:#ef4444; font-weight:700;">• 🚨 ${r._diffDays} Gün</span>` : ''}
              </div>
            </div>
          </div>

          <div style="display: flex; align-items: center; gap: 0.65rem; flex-shrink: 0;">
            ${this.getStatusBadge(r)}
            <div style="font-family: var(--font-mono); font-weight: 700; font-size: 0.84rem; color: var(--text-main); text-align: right; min-width: 80px;">
  ...
  renderDashboardActionAlerts(requests, contracts, invoices, guarantees, allNotifs) {
    const alertsBox = document.getElementById('dash-action-alerts-list');
    if (!alertsBox) return;

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // 1. Overdue requests (14+ days)
    const overdueReqs = requests.filter(r => r.status !== 'Tamamlandı' && r.status !== 'Reddedildi' && r.status !== 'İptal' && r._diffDays >= 14);

    // 2. Expiring / Overdue guarantees
    const activeGuarantees = (guarantees || []).filter(g => {
      const st = (g.status || '').toLowerCase();
      return st !== 'iade edildi' && st !== 'iade' && st !== 'nakte çevrildi' && st !== 'iptal' && st !== 'hükümsüz' && st !== 'kapandı';
    });
    let criticalGuaranteesCount = 0;
    activeGuarantees.forEach(g => {
      const exp = this.parseDate(g.expiryDate);
      if (exp) {
        exp.setHours(0, 0, 0, 0);
        const diff = Math.ceil((exp - today) / (1000 * 60 * 60 * 24));
        if (diff <= 30) criticalGuaranteesCount++;
      }
    });

    // 3. Invoices pending delivery or overdue
    let pendingDeliveryCount = 0, pendingDeliveryAmount = 0;
    (invoices || []).forEach(inv => {
      if (!inv.accountingDeliveryDate && inv.paymentStatus !== 'Ödendi') {
        pendingDeliveryCount++;
        pendingDeliveryAmount += (inv.amount || 0);
      }
    });
    ...
    // Alert 1: Overdue Requests
    if (overdueReqs.length > 0) {
      items.push(`
        <div class="dash-compact-alert-item alert-red" onclick="App.filterRequestsByDashboard('OVERDUE_14')" title="14+ Gündür Bekleyen Talepleri Filtrele">
          <div style="display: flex; align-items: center; gap: 0.6rem;">
            <span style="font-size: 1.15rem;">🚨</span>
            <div>
              <div style="font-size: 0.82rem; font-weight: 700; color: var(--status-rejected);">
                ${overdueReqs.length} Talep 14+ Günü Aştı (SLA Alarmı)
              </div>
              <div style="font-size: 0.72rem; color: var(--text-muted);">Süresi uzayan dosyalara müdahale edilmesi önerilir.</div>
            </div>
          </div>
          <span class="badge priority-kritik" style="font-size: 0.7rem; font-weight: 700;">Gecikmede</span>
        </div>
      `);
    }

    // Alert 2: Guarantees
    if (criticalGuaranteesCount > 0) {
      items.push(`
        <div class="dash-compact-alert-item alert-yellow" onclick="App.switchView('guarantees')" title="Teminat Mektuplarını İncele">
          <div style="display: flex; align-items: center; gap: 0.6rem;">
            <span style="font-size: 1.15rem;">🛡️</span>
            <div>
              <div style="font-size: 0.82rem; font-weight: 700; color: #d97706;">
                ${criticalGuaranteesCount} Teminat Mektubu Vadesi Yaklaşıyor
              </div>
              <div style="font-size: 0.72rem; color: var(--text-muted);">Önümüzdeki 30 gün içinde vadesi dolacak mektuplar.</div>
            </div>
          </div>
          <span class="badge priority-orta" style="font-size: 0.7rem; font-weight: 700;">Vade Takibi</span>
        </div>
      `);
    }
    ...
    if (pendingDeliveryCount > 0) {
      items.push(`
        <div class="dash-compact-alert-item alert-blue" onclick="App.switchView('invoices')" title="Faturalar Sayfasına Git">
          <div style="display: flex; align-items: center; gap: 0.6rem;">
            <span style="font-size: 1.15rem;">🧾</span>
            <div>
              <div style="font-size: 0.82rem; font-weight: 700; color: var(--accent-primary);">
                ${pendingDeliveryCount} Fatura Muhasebeye Teslim Bekliyor
              </div>
              <div style="font-size: 0.72rem; color: var(--text-muted);">Toplam: ${this.formatMoney(pendingDeliveryAmount, 'TRY', 0)}</div>
            </div>
          </div>
          <span class="badge status-open" style="font-size: 0.7rem; font-weight: 700;">Teslimat</span>
        </div>
      `);
    }
    ...
    if (expiringContractsCount > 0) {
      items.push(`
        <div class="dash-compact-alert-item alert-yellow" onclick="App.switchView('contracts')" title="Sözleşmeleri İncele">
          <div style="display: flex; align-items: center; gap: 0.6rem;">
            <span style="font-size: 1.15rem;">📑</span>
            <div>
              <div style="font-size: 0.82rem; font-weight: 700; color: #d97706;">
                ${expiringContractsCount} Sözleşmenin Bitiş Süresi Yaklaşıyor
              </div>
              <div style="font-size: 0.72rem; color: var(--text-muted);">Yenileme ve ihale hazırlığı gerekebilir.</div>
            </div>
          </div>
          <span class="badge priority-orta" style="font-size: 0.7rem; font-weight: 700;">Yenileme</span>
        </div>
      `);
    }

    // Always include: Personnel Workload pulse item
    items.push(`
      <div class="dash-compact-alert-item alert-green" onclick="App.switchView('workload')" title="İş Yükü ve Delegasyon Sayfasını Aç">
        <div style="display: flex; align-items: center; gap: 0.6rem;">
          <span style="font-size: 1.15rem;">👥</span>
          <div>
            <div style="font-size: 0.82rem; font-weight: 700; color: var(--status-completed);">
              ${activeUsers.length} Aktif Uzman • Ort. ${avgLoad} Açık Dosya
            </div>
            <div style="font-size: 0.72rem; color: var(--text-muted);">Ekip iş yükü ve kota dağılımı stabil durumda.</div>
          </div>
        </div>
        <span class="badge status-completed" style="font-size: 0.7rem; font-weight: 700;">Dengeli</span>
      </div>
    `);

    // If no critical alerts at all
    if (overdueReqs.length === 0 && criticalGuaranteesCount === 0 && pendingDeliveryCount === 0 && expiringContractsCount === 0) {
      items.unshift(`
        <div class="dash-compact-alert-item alert-green" style="cursor: default;">
          <div style="display: flex; align-items: center; gap: 0.6rem;">
            <span style="font-size: 1.15rem;">✨</span>
            <div>
              <div style="font-size: 0.82rem; font-weight: 700; color: var(--status-completed);">
                Tüm Süreçler Yolunda
              </div>
              <div style="font-size: 0.72rem; color: var(--text-muted);">Geciken talep veya süresi yaklaşan kritik uyarı bulunmuyor.</div>
            </div>
          </div>
          <span class="badge status-completed" style="font-size: 0.7rem;">Normal</span>
        </div>
      `);
    }
    ...
    if (body) {
      body.innerHTML = `
        ...
        <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 1.25rem; margin-bottom: 1.25rem; display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.25rem; font-size: 0.9rem;">
          ...
          <div>
            <div style="font-size: 0.75rem; color: var(--text-muted); font-weight: 600; margin-bottom: 0.25rem;">BÜTÇE / MUHASEBE KALEMİ</div>
            <div style="font-weight: 600; color: var(--text-main);">${req.budgetItem ? `📊 ${req.budgetItemCode ? `<code style="background:rgba(99,102,241,0.12); color:var(--accent-primary); padding:2px 6px; border-radius:4px; font-weight:700;">${req.budgetItemCode}</code> ` : ''}${req.budgetItem}` : '<span style="color:var(--text-muted);">-</span>'}</div>
          </div>
          <div>
  ...
  setSettingsTab(tabName) {
    ...
    } else if (tabName === 'budget-items') {
      this.renderBudgetItemsSettings();
  ...
  // ============================================================
  // 📊 BÜTÇE KALEMLERİ YÖNETİMİ (SETTINGS TAB)
  // ============================================================
  renderBudgetItemsSettings() {
    const tbody = document.getElementById('tbody-budget-items-settings');
    if (!tbody) return;
    const searchVal = (document.getElementById('filter-budget-items-search')?.value || '').toLowerCase().trim();
    const items = this.state.budgetItems || [];
    
    const filtered = items.filter(it => {
      if (!searchVal) return true;
      const matchCode = (it.code || '').toLowerCase().includes(searchVal);
      const matchName = (it.name || '').toLowerCase().includes(searchVal);
      const matchCat = (it.category || '').toLowerCase().includes(searchVal);
      return matchCode || matchName || matchCat;
    });

    if (filtered.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--text-muted); padding:1.5rem;">Kayıtlı bütçe kalemi bulunamadı.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = filtered.map(item => {
      ...
      const statusBadge = isAct
        ? `<span class="badge status-completed" style="font-size:0.75rem;">🟢 Aktif</span>`
        : `<span class="badge status-open" style="font-size:0.75rem; background:rgba(148,163,184,0.15); color:var(--text-muted);">⚪ Pasif</span>`;

      return `
        <tr>
          <td><code style="font-family:var(--font-mono); font-weight:700; color:var(--accent-primary); background:rgba(99,102,241,0.08); padding:3px 6px; border-radius:4px;">${item.code || '-'}</code></td>
          ...
          <td style="text-align:center;">
            <div class="action-btns" style="justify-content:center; gap:0.25rem;">
              <button class="btn-icon" onclick="App.openBudgetItemModal(${item.id})" title="Kalemi Düzenle">✏️</button>
              <button class="btn-icon" onclick="App.handleDeleteBudgetItem(${item.id}, '${item.name.replace(/'/g, "\\'")}')" title="Kalemi Sil">🗑️</button>
            </div>
          </td>
        </tr>
      `;
  ...
  openBudgetItemModal(id = null) {
    const titleEl = document.getElementById('bif-modal-title');
    const idInput = document.getElementById('bif-id');
    const codeInput = document.getElementById('bif-code');
    const nameInput = document.getElementById('bif-name');
    const catInput = document.getElementById('bif-category');
    const statusSelect = document.getElementById('bif-status');

    if (id) {
      const item = (this.state.budgetItems || []).find(b => String(b.id) === String(id));
      if (!item) return;
      if (titleEl) titleEl.innerText = `✏️ Bütçe Kalemini Düzenle (${item.code})`;
      if (idInput) idInput.value = item.id;
      if (codeInput) codeInput.value = item.code || '';
      if (nameInput) nameInput.value = item.name || '';
      if (catInput) catInput.value = item.category || '';
      if (statusSelect) statusSelect.value = item.isActive !== false ? 'true' : 'false';
    } else {
      if (titleEl) titleEl.innerText = '📊 Yeni Bütçe Kalemi Tanımla';
    ...
    this.openModal('modal-budget-item-form');
  },

  async handleSaveBudgetItem(e) {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    const id = document.getElementById('bif-id')?.value;
    const code = document.getElementById('bif-code')?.value.trim();
    const name = document.getElementById('bif-name')?.value.trim();
    const category = document.getElementById('bif-category')?.value.trim() || '';
    const isActive = document.getElementById('bif-status')?.value === 'true';

    if (!code || !name) {
      this.showToast("Bütçe kodu ve kalem adı zorunludur!", "warning");
      return;
    }

    const payload = { code, name, category, isActive };

    try {
      if (id) {
        const updated = await this.apiSync('budget_items', 'PUT', payload, id);
        const idx = (this.state.budgetItems || []).findIndex(b => String(b.id) === String(id));
        if (idx !== -1) {
          this.state.budgetItems[idx] = { ...this.state.budgetItems[idx], ...payload, ...updated };
        }
        this.showToast(`"${name}" bütçe kalemi güncellendi.`, "success");
      } else {
        const created = await this.apiSync('budget_items', 'POST', payload);
        this.state.budgetItems.push(created || payload);
        this.showToast(`"${name}" bütçe kalemi eklendi.`, "success");
      }

      this.closeModal('modal-budget-item-form');
      this.renderBudgetItemsSettings();
      this.populateDropdowns();
    } catch (err) {
      console.error('Bütçe kalemi kaydetme hatası:', err);
      this.showToast(`Bütçe kalemi kaydedilemedi: ${err.message}`, "error");
    }
  },

  async handleDeleteBudgetItem(id, name) {
    this.showConfirm("Bütçe Kalemini Sil", `"${name}" bütçe kalemini silmek istediğinizden emin misiniz?`, async () => {
      try {
        await this.apiSync('budget_items', 'DELETE', null, id);
        this.state.budgetItems = (this.state.budgetItems || []).filter(b => String(b.id) !== String(id));
        this.renderBudgetItemsSettings();
        this.populateDropdowns();
        this.showToast(`"${name}" bütçe kalemi silindi.`, "info");
      } catch (err) {
        console.error(err);
  ...
  // ============================================================
  // 📊 BÜTÇE KALEM DAĞILIMI VE HARCAMA ANALİZİ MODALI
  // ============================================================
  openBudgetBreakdownModal(targetUnitName = null) {
    const isUnitUser = this.state.currentUser?.role === 'UNIT';
    const unitName = targetUnitName || (isUnitUser ? this.state.currentUser?.unit : document.getElementById('filter-unit')?.value) || (this.state.currentUser?.unit && this.state.currentUser?.unit !== 'Genel Satınalma' ? this.state.currentUser?.unit : 'ALL');
    const currentYear = this.state.selectedYear || getCurrentAcademicYear();

    this.state.currentBreakdownUnit = unitName;
    this.state.currentBreakdownYear = currentYear;

    const titleEl = document.getElementById('bib-title');
    const subEl = document.getElementById('bib-subtitle');
    const isAll = !unitName || unitName === 'ALL';

    if (titleEl) {
      titleEl.innerText = isAll 
        ? `📊 Kurum Geneli Bütçe Kalem Dağılımı (${currentYear})`
        : `📊 ${unitName} — Bütçe Kalem Dağılımı (${currentYear})`;
    }
    if (subEl) {
      subEl.innerText = isAll
        ? `Tüm birimlerin ${currentYear} akademik yılındaki açık ve gerçekleşen harcamalarının analitik bütçe kalemlerine göre dağılımı`
        : `${unitName} biriminin ${currentYear} akademik yılındaki açık ve gerçekleşen harcamalarının analitik bütçe kalemlerine göre dağılımı`;
    }

    const searchInput = document.getElementById('bib-search');
    if (searchInput) searchInput.value = '';

    this.renderBudgetBreakdownTable('');
    this.openModal('modal-budget-items-breakdown');
  },

  renderBudgetBreakdownTable(searchQuery = '') {
    const unitName = this.state.currentBreakdownUnit || 'ALL';
    ...
    // 2. Filter requests by academic year & unit
    let reqs = (this.state.requests || []).filter(r => {
      const rYear = r.academicYear || this.getAcademicYear(r.arrivalDate || r.requestDate);
      if (currentYear !== 'ALL' && rYear !== currentYear) return false;
      if (!isAll && (r.unit || '').trim().toLowerCase() !== unitName.trim().toLowerCase()) return false;
      return true;
    });

    const committedReqs = reqs.filter(r => r.status !== 'Tamamlandı' && r.status !== 'İptal Edildi' && r.status !== 'Reddedildi');
    const spentReqs = reqs.filter(r => r.status === 'Tamamlandı');

    const totalCommitted = committedReqs.reduce((sum, r) => sum + (parseFloat(r.actualAmount || r.budgetAmount || r.estimatedAmount) || 0), 0);
    const totalSpent = spentReqs.reduce((sum, r) => sum + (parseFloat(r.actualAmount) || 0), 0);
    const totalLoad = totalCommitted + totalSpent;
    const available = totalBudget - totalLoad;

    // Update mini KPIs
    const kpiBudget = document.getElementById('bib-kpi-total-budget');
    const kpiComm = document.getElementById('bib-kpi-committed');
    const kpiSpent = document.getElementById('bib-kpi-spent');
    const kpiAvail = document.getElementById('bib-kpi-available');

    if (kpiBudget) kpiBudget.innerText = totalBudget > 0 ? this.formatMoney(totalBudget, 'TRY', 0) : '0 ₺';
    if (kpiComm) kpiComm.innerText = this.formatMoney(totalCommitted, 'TRY', 0);
    if (kpiSpent) kpiSpent.innerText = this.formatMoney(totalSpent, 'TRY', 0);
    if (kpiAvail) {
      kpiAvail.innerText = this.formatMoney(available, 'TRY', 0);
    ...
    const tbody = document.getElementById('tbody-budget-breakdown');
    if (!tbody) return;
  ...
  filterBudgetBreakdownTable() {
    const q = document.getElementById('bib-search')?.value || '';
    this.renderBudgetBreakdownTable(q);
  },

  exportBudgetBreakdownToExcel() {
    const data = this.state.lastBudgetBreakdownData;
    if (!data || !data.items || data.items.length === 0) {
      this.showToast("Dışa aktarılacak bütçe kalemi verisi bulunamadı.", "warning");
      return;
    }

    const rows = [
      ['Bütçe ve Harcama Kalem Dağılım Raporu — Piri Reis Üniversitesi'],
    ...
    if (typeof XLSX !== 'undefined') {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Butce_Kalem_Dagilimi");
      const safeUnit = (data.unitName || 'Kurum_Geneli').replace(/[^a-zA-Z0-9_\u00C0-\u017F]/g, '_');
      XLSX.writeFile(wb, `Butce_Kalem_Dagilimi_${safeUnit}_${data.academicYear}.xlsx`);
      this.showToast("Bütçe kalem dağılımı Excel dosyası olarak indirildi.", "success", "📥");
    } else {
      this.showToast("Excel kütüphanesi yüklenemedi!", "error");
    }
  },
  ...
  renderUnitBudgetWidget() {
    if (this.state.currentView === 'requests') {
      this.renderRequestsTable();
    }
  },
  ...
  async handleNewRequest(e) {
    ...
    const budgetItemSelect = document.getElementById('nr-budget-item');
    const budgetItem = budgetItemSelect?.value || '';
    const budgetItemCode = budgetItemSelect?.options[budgetItemSelect.selectedIndex]?.getAttribute('data-code') || '';
  ...
  async handleEditRequest(e) {
    ...
    const budgetItemSelect = document.getElementById('er-budget-item');
    const budgetItem = budgetItemSelect?.value || '';
    const budgetItemCode = budgetItemSelect?.options[budgetItemSelect.selectedIndex]?.getAttribute('data-code') || '';
    ...
    if (multiSuppliers.length > 1) {
      ...
      req.budgetItemCode = budgetItemCode;
